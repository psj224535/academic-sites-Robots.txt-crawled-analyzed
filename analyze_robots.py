...
def parse_robots_txt(content):
    ...
    for line in content.split('\n'):
        ...
        if lower_line.startswith('user-agent:'):
            agent = line[11:].strip()
            # If previous line wasn't a user-agent, this is a new block
...
def analyze_bot_status(bot_name, policies):
    ...
    # Filter out empty strings from disallow
...
def process_file(filepath, name, category, url):
    ...
    for group, bots in bots_to_check.items():
        for bot in bots:
            status = bot_policies_out[group][bot]
            if status in ["Blocked", "Partially Blocked"]:
                 notable.append(f"{bot} is {status}")
            elif status == "Allowed":
...
def main():
    with open('raw_robots/metadata.json', 'r', encoding='utf-8') as f:
        metadata = json.load(f)
        
    results = []
    for name, info in metadata.items():
        if info['status'] == 'success':
            filepath = os.path.join('raw_robots', f"{name}.txt")
            if os.path.exists(filepath):
                res = process_file(filepath, name, info['category'], info['url'])
                results.append(res)
